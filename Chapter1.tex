...
\title{Solutions for Chapter 1}
    ...
    \ex{1.21}
    Two circuits under consideration: (a) center-tap transformer with 2 diodes and (b) center-tap transformer with full-bridge rectifier. In both cases, our load is 1A.
    \begin{circuit}{fig:1.17.1}{(a) Center-tap transformer, full-wave rectifier circuit}
        (0,0) node[transformer](T){}
        (T.A2) -- ++(-1,0) coordinate (aux)
        to[vsourcesin] (aux |- T.A1)
        --(T.A1);
        \draw (T.B1) to[diode] ++(3,0) node[ocirc]{} coordinate(aux2)
        (T.B2) to[diode] ++(3,0) node[ocirc]{} coordinate(aux1)
        to[isource] (aux2 -| aux1);
    \end{circuit}
